...
def GPS_Sync(sat_detected, signal, reference_table):
    """ this function takes in our known sat, and signal receved, and gets binary back."""
    print("\nThread for sat {}".format((sat_detected + 1)))
    window_end = 1023
    window_start = 0
    negs, posi = 0, 0
    synced = 0
    ...
    while window_end <= len(signal):
        ...
        if R_value < -0.85:
            ...
            negs, posi = 0, 0
        print("Synced,{} Window_end,{} negs,{} posi,{}".format(synced, window_end, negs, posi))
        window_end += 1
        window_start += 1
    ...
    while window_end <= len(signal):
        ...
        if negs == 20 or posi == 20:
            ...
            if negs == 20:
                ...
                working_word.append(1)
            negs, posi = 0, 0
        print("Synced,{} Window_end,{} negs,{} posi,{} R,{} ".format(synced, window_end, negs, posi, R_value))
    binary_string = Utility_functions.stringify(working_word)
    print(binary_string)
    return binary_string
...
def multi_sat_handler(sats_detected, signal, reference_table):
    ...
    while count < len(sats_detected):
        ...
        message_received.append(PRN_reader(sats_detected[count],signal,reference_table))
        count += 1
